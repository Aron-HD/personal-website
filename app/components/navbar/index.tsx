...
const NavClosed = ({ scrollFuncs }: Partial<ScrollFuncsProps>) => {
  ...
  const [scrollPercent, setScrollPercent] = useState(61)
  ...
  const handleScroll = () => {
    ...
    if (p > 61 && p < 80) setScrollPercent(p)
  ...
  return (
    <nav className={`${scrollingDown ? "opacity-0" : "opacity-100"} ${scrolledToTop ? "sticky opacity-100" : "fixed"} 
      flex w-full items-center justify-between m-auto p-[10px_40px_1.0875rem] ease-linear duration-300 transition-[all_0.25s_cubic-bezier(0.65,_0.05,_0.36,_1)]`
    } >
